...
@Component({
  ...
  template: `
    <div [ngClass]="alertClasses" class="rounded-md p-4 mb-4 flex justify-between items-center animate-fade-in">
      <div class="flex items-center">
        <div [ngClass]="iconClasses" class="mr-3">
        ...
        <p [ngClass]="textClasses" class="text-sm font-medium">{{ message }}</p>
      </div>
      <button (click)="onDismiss.emit()" class="text-gray-400 hover:text-gray-600">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
...
export class AlertComponent {
  ...
  get alertClasses() {
    return {
      'bg-green-50 border border-green-100': this.type === 'success',
      'bg-red-50 border border-red-100': this.type === 'error',
      'bg-yellow-50 border border-yellow-100': this.type === 'warning',
      'bg-blue-50 border border-blue-100': this.type === 'info'
    };
  }
